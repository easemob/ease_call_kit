...
class EaseCallKit {
  ...
  static Future<void> initWithConfig(EaseCallConfig config) async {
    ...
    req['userMap'] = config.userMap?.keys
  ...
  static Future<String> startSingleCall(
    ...
    req['ext'] = ext;
    Map result = await _channel.invokeMethod('initWithConfig', req);
    String callId;
    if (result['error']) {
...
class EaseCallError {
  ...
  factory EaseCallError.fromJson(Map map) {
    EaseCallErrorType callType = EaseCallErrorType.IM
    return EaseCallError()..errCode = map['err_code']..errDescription = map['err_desc']..errType = ;
  }
}
...
enum EaseCallErrorType {Noral, Process, RTC, IM }
